...
export const ResumoVendasEntity: IResumoVendasEntity = {
  async create(ctx, input, runtime) {
    ...
    const repo = await data.moduleData.getTable<ResumoVendasRecord>('sales_summary_requests');
  ...
  async getById(ctx, salesSummaryRequestId, runtime) {
    ...
    if (!record) {
      throw new AppError('NOT_FOUND', 'Sales summary request not found', 404, { id: salesSummaryRequestId });
    }
    return record;
  },
