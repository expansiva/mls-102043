...
const message_pt = {
  ...
  couldNotAtualizarStatusPedido: 'Nao foi possivel atualizar status do pedido',
};
const message_en = {
  brand: 'Cafe Flow',
...
export class PainelCozinhaPainelCozinhaBase extends CollabLitElement {
  ...
  handleIcaStateChange(key: string, value: any): void {
    switch (key) {
      ...
      case 'ui.painelCozinha.atualizarStatusPedido':
        this.atualizarStatusPedidoState = value ?? 'idle';
        break;
      default:
        break;
    }
  ...
  handleAtualizarStatusPedidoClick(): void {
    const first = this.pedidosCozinha[0];
    const params = {
      orderId: first?.orderId ?? 'id-001',
      novoStatus: first?.status ?? 'Em preparo',
    };
